...
def get_version_diff():
    entity_type = request.args.get("entity_type")
    entity_id = request.args.get("entity_id", type=int)
    va = request.args.get("version_a", type=int) or request.args.get("v1", type=int)
    vb = request.args.get("version_b", type=int) or request.args.get("v2", type=int)

    if not entity_type or not entity_id or not va or not vb:
        return jsonify({"error": "entity_type, entity_id, version_a/v1, and version_b/v2 are required"}), 400
